...
- (void)connectToGainDataWithPage:(NSInteger)page {
    ...
    [[ConnectManager sharedManager] getDataWithURL:[NSString stringWithFormat:@"http://kapi.zucaitong.com/news?last_id=0&page_size=%ld&type=2", 20 * page] params:nil success:^(id responseObj) {
        if (responseObj) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NewsModel * nModel = self.allDataArray[indexPath.row];
    VideosContentViewController *vcVC = [[VideosContentViewController alloc]init];
     vcVC.myId = nModel.newsId;
